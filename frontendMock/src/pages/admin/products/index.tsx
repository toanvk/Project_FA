...
const AdminProducts: React.FC = () => {
  ...
  const columns: ColumnsType<IProduct> = [
    ...
    },
    {
      ...
      render: (_, record) => {
        return (
          <img
            className="w-40 h-24 border-solid border-1 border-[#7675756f] rounded-md"
            src={
              record.image
                ? record.image
    ...
    },
    {
      ...
      render: (_, record) => {
        return (
          <Space size="middle">
            <Button onClick={() => buttonUpdate(record.slug.toString())}>
              Sửa
            </Button>
            <Button
              onClick={() => navigate(`/product/${record.slug.toString()}`)}
            >
              Xem
            </Button>
          </Space>
  ...
  return (
    <div>
      <Button
        href={routes.ADMIN_PRODUCTS_ADDNEW}
        className="mb-4"
        danger
        icon={<PlusOutlined />}
      >
