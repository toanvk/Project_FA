...
const EditProduct: React.FC = () => {
  ...
  const deleteMetadata = async () => {
    ...
    if (metadataId) {
      try {
        await deleteMetadataById(metadataId)
          .then((data: string) => {
            console.log(data);
            const metadata = metadataInGroup.filter(
  ...
  useEffect(() => {
    if (param) {
      getProductBySlug(param.slug).then((data: IProduct) => {
        ...
        getProductImage(data.id).then((data: IProductImage[]) => {
      ...
      {product && (
        ...
        >
          ...
          <div className="flex space-x-2">
                          ...
                          >
                            <div className="flex space-x-2 items-center">
                              ...
                              <Button
                                ...
                                icon={<EditOutlined />}
                                onClick={async () => {
                                  await showEditMetadata(metadata.id);
                                }}
                              >
                                Sửa
                              </Button>
                              <Button
                                ...
                                icon={<DeleteOutlined />}
                                onClick={() => {
                                  handleDeleteMetadata(metadata.id);
                                }}
                              >
                                Xóa
                              </Button>
            ...
            <div className="flex-1">
              <div className="font-bold text-lg mb-3">FAQs</div>
              {faqs.length > 0 &&
                faqs.map((faq) => (
                  <div
                    key={faq.id}
              ...
              <div className="my-3 font-bold">Xem trước</div>
              <FAQs listFaq={faqs} />
            </div>
          </div>
          ...
          <div className="flex space-x-2">
            ...
            >
              <div className="flex space-x-2 justify-center items-center">
                ...
                <IconSelectionModal
                  visible={selectIconEdit}
                  onClose={() => {
                    setSelectIconEdit(false);
                  }}
                  onSelectIcon={handleSelectionIcon}
                />
              </div>
