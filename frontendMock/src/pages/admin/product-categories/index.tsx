...
const ProductCategories: React.FC = () => {
  ...
  const [productCategoryDelete, setProductCategoryDelete] =
    useState<string>("");
  ...
  const fetchData = async () => {
    setLoading(true);
    await getAllProductCategories().then((data: any) => {
      console.log(data);
      setDataSource(data);
      setLoading(false);
    });
  ...
  return (
    <Spin tip="Loading...." spinning={loading}>
      <Button
        href={routes.ADMIN_PRODUCTCATEGORIES_ADDNEW}
        className="mb-4"
        danger
        icon={<PlusOutlined />}
      >
      ...
      >
        <div className="flex flex-col items-center">
          Tất cả sản phẩm trong danh mục này sẽ tự động chuyển sang danh mục mặc
          định!
        </div>
      </Modal>
    </Spin>
