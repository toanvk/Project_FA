...
const SettingFooter: React.FC = () => {
  ...
  const handleDragSubMenu = (result: DropResult) => {
    ...
    if (type === "submenu") {
      ...
      const destinationMenu = updatedMenus.find(
        (menu) => menu.id === destinationMenuId
      );
  ...
  return (
    <>
      <div className="flex items-center space-x-4 mb-3">
        <div className="font-bold text-lg">Kéo thả để sắp xếp menu</div>
        {totalMenu < 4 && (
          <Button
            danger
            size="small"
          ...
          <div className="flex flex-col">
            ...
            <Form.Item label="Name">
              <Input
                ...
                placeholder="Dịch vụ"
                onChange={(e) => {
                  setEditDataName(e.target.value);
                }}
              />
            </Form.Item>
            <Form.Item label="URL">
              <Input
                value={editDataUrl}
                placeholder="https://google.com or /blogs"
                onChange={(e) => {
                  setEditDataUrl(e.target.value);
                }}
              />
            </Form.Item>
            <Form.Item label="Icon">
              <div className="flex items-center space-x-2">
                <ShowIcon
                  name={editDataIcon || "FcAddImage"}
                  size={34}
                  onClick={() => {
                    setShowIconOnEdit(true);
                  }}
                />
                <IconSelectionModal
                  visible={showIconOnEdit}
                  onClose={() => {
                    setShowIconOnEdit(false);
                  }}
                  onSelectIcon={setEditDataIcon}
                />
              </div>
            </Form.Item>
            <div>
                        ...
                        {(provided, snapshot) => (
                          <div className="flex items-center space-x-2">
                            ...
                            <Button
                              ...
                              icon={<EditOutlined />}
                              size="small"
                              onClick={() => handleEdiSubMenuClick(submenu)}
                            ></Button>
                            <DeleteModal
                              id={submenu.id.toString()}
                              onDelete={deleteSubMenuById}
                              onSuccess={() => {
                                message.success("Xóa thành công!");
                              }}
                            />
