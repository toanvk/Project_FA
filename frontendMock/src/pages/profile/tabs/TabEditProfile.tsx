...
const TabEditProfile: React.FC = () => {
  ...
  const doUpdatePassword = () => {
    form2.validateFields().then((values: any) => {
      console.log(values);
  ...
  const onFinish = (values: any) => {
    ...
    if (user) {
      ...
      console.log("onFinish", image, userEdit.image);
      if (token) {
        updateProfile(token, userEdit).then(() => {
  ...
  useEffect(() => {
    if (username) {
      getUserInfo(username).then((data: IUser) => {
        setUser(data);
          ...
          <div className="flex">
            <div className="flex flex-col space-y-4 items-center">
              <div className="w-32 h-32 bg-gray-200 rounded-full flex justify-center">
                <Image
                  className="w-32 h-32 bg-gray-200 rounded-full flex justify-center"
                  src={
                    user?.image ||
              ...
              <div className="md:ml-5 md:pl-5 md:border-0 md:border-l md:border-solid md:border-[#d9d9d9]">
                <div>
                  ...
                  <div className="flex space-x-2 items-center">
                    <LockOutlined /> <div>*******</div>{" "}
                    <Button danger onClick={setShowChangePassword} size="small">
                      Cập nhật
                    </Button>
                  </div>
                </div>
