...
const Payment: React.FC = () => {
  ...
  useEffect(() => {
    console.log("ORDER1");
    if (username && param) {
      getOrderByUserName(username).then((data) => {
        console.log("ORDER", data);
        const checkExistId = data.some(
          (item: any) => Number(item.id) === Number(param.orderid)
        );
        if (checkExistId) {
        ...
        <Card className="container mx-auto py-4 md:w-[600px]">
          <div className="flex space-x-4">
            <div className="flex flex-col items-center">
              <QRCode errorLevel="H" value={qrCode} icon={TPBank} />
            </div>
            <div className="flex flex-col space-y-3">
              <div className="flex flex-col">
                <div className="font-bold text-2xl">Order #{param.orderid}</div>
                <div>Desc order</div>
              </div>
              ...
              <div>
                ...
                <span className="text-red-500 font-bold">{`SWPORDER${param.orderid}`}</span>
              </div>
              <div>
                <Button
                  onClick={() => {
                ...
                <div className="mt-2">
                  Order tự động chuyển sang trạng thái DONE khi chúng tôi đã
                  nhận được tiền!
                </div>
              </div>
            </div>
