  ...
  const renderItems = () => {
    return laptops.map((data: IProduct) => (
      <Link
        ...
        key={data.id}
        className="flex flex-col justify-start rounded-lg overflow-hidden shadow-lg hover:scale-105 transition duration-500 bg-white no-underline"
      >
        <div className="relative">
          <img
            className="absolute top-0 left-0 bottom-0 right-0 w-full h-48"
            src="https://images.fpt.shop/unsafe/fit-in/270x210/filters:quality(90):fill(white)/fptshop.com.vn/Uploads/Originals/2023/7/1/638237980035622461_frame-cate-270x210.png"
            alt=""
          />
          <img
            ...
            alt=""
          />
          <div className="absolute left-0 bottom-4 text-[14px] text-white py-1 px-3 rounded-r-md bg-[#ea9d02]">
            Trả góp 0%
          </div>
        </div>
        <div className="py-4 px-4 flex flex-col justify-between align-bottom">
          <div className="text-sm font-semibold text-[#CD1818]">
            Mã SP: <span className="uppercase">{data.sku}</span>
          </div>
          <div
            className="text-[17px] font-bold text-[#474c51] mb-2 h-[37px] block overflow-hidden"
          ...
          <div className="flex justify-between items-center pb-2">
            <div
              className="relative text-base font-mono font-extrabold bg-[#ef8573] text-white px-2 rounded-full basis-[130px] w-[130px] h-[28px] flex justify-center items-center"
              style={{ zIndex: 1 }}
            >
              {formatCurrency(data.price - (data.price * data.discount) / 100)}
              <div
                className={`absolute top-0 bottom-0 left-0 bg-[#CD1818] rounded-full`}
                style={{ zIndex: -1, width: `${100 - data.discount}%` }}
              ></div>
            </div>
            <div
              className={`text-base text-[#919191] text-[14px] ${
                data.discount > 0 ? "text-decoration: line-through" : ""
              }`}
            >
            ...
            <div className="flex-1">
              ...
              <div className="mb-5">
                ...
                <div className="pr-5">
                  ...
                  <Button onClick={onFilterPrice} className="w-full my-3">
                    Lọc
                  </Button>
                </div>
              </div>
