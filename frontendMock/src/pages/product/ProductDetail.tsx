import { Button, Card, Divider, InputNumber, Modal, Spin, message } from "antd";
...
const ProductDetail: React.FC = () => {
    ...
    if (data) {
      ...
      const lpPromises = listProductCart.map(async (item: IProductCart) => {
        ...
        const product: IProductCart = {
          ...
          discount: data.discount,
          quantity:
            item.quantity > data.quantity ? data.quantity : item.quantity,
        };
      ...
      Promise.all(lpPromises).then((results) => {
  ...
  const removeProduct = (id: number) => {
    const updatedProductList = listProductCart.filter(
  ...
  const updateCart = (id: number, quantity: number | null) => {
    ...
    if (
      quantity !== null &&
      !listProductCart.some((product) => product.id === id)
    ) {
  ...
  const openDetail = async () => {
    await getMetadataWithMetadataGroup(param.slug).then(
      (data: IMetadataGroup[]) => {
        setDataDetail(data);
        setShowDetail(true);
      }
  ...
  useEffect(() => {
    if (param) {
      ...
      getProductBySlug(param.slug).then((data: IProduct) => {
        setData(data);
    ...
    <>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        <div className="flex flex-col md:flex-row -mx-4 bg-white rounded-md">
          ...
          <div className="md:flex-1 px-4 p-5">
            <div className="font-bold uppercase text-2xl text-[#CD1818]">
              Giá cũ: {data.price && data.price.toLocaleString()} VNĐ
            </div>
            <div className="font-bold uppercase text-2xl text-[#CD1818]">
              Giá khuyến mại:{" "}
              {(
            ...
            <div className="flex py-4 space-x-4">
              <Button
                danger
                className="h-14 px-6 py-2 font-semibold rounded-xl"
        ...
        <div className="md:flex justify-between md:space-x-4 -mx-4">
          <div className="md:w-2/3 flex flex-col md:flex-row mt-5 bg-white rounded-md p-5">
            <div
              dangerouslySetInnerHTML={{ __html: data.summary || "No content" }}
            />
          </div>
        ...
        <Card title="Đánh giá sản phẩm" className="bg-white -mx-4 my-4">
          <div className="flex justify-around">
            <div className="review-block1"></div>
      ...
      <Modal
        ...
        footer={null}
      >
        <div className="p-5">{dataDetail && renderDetailData(dataDetail)}</div>
      </Modal>
            ...
            {listProductCart.length > 0 ? (
              <>
                ...
                <div className="flex justify-center">
                  <Button danger size="large" onClick={createNewOrder}>
                    {" "}
                    HOÀN TẤT ĐẶT HÀNG{" "}
                  </Button>
                </div>
